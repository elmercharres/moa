...
variable "alb_subnet_ids" {
  description = "Subnet IDs for the Application Load Balancer. Use public subnets for internet-facing ALBs; private subnets for internal ALBs."
  type        = list(string)
}

variable "alb_ingress_cidr_blocks" {
  description = "CIDR blocks allowed to reach the ALB. Restrict to internal network ranges in non-public deployments."
  type        = list(string)
  default     = ["10.0.0.0/8"]
}
